...
| D1 + Drizzle | DynamoDB on-demand, single-table design keyed by entity access patterns. Ported so far: [`aws/db-identity/`](../aws/db-identity/) (`UserService`/`ApiKeyService`, 20 tests), [`aws/db-apps/`](../aws/db-apps/) (core of `AppService`, 26 tests — trending/popular ranking, fork-detachment, and comments/likes explicitly not ported, see its README), and [`aws/db-model-config/`](../aws/db-model-config/) (`ModelProvidersService` + the storage layer of `ModelConfigService`, 16 tests — merge-with-defaults/constraint logic explicitly not ported, it's application logic, not storage) against [docs/aws-dynamodb-schema.md](aws-dynamodb-schema.md). Remaining 4 services (`AuthService`, `SessionService`, `AnalyticsService`, `ModelTestService`) not yet ported. | Medium-High — no relational engine, so this is a query-layer rewrite, not a dialect swap (see decision above) |
| R2 | S3 | Low |
