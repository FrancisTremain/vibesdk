...
| D1 + Drizzle | DynamoDB on-demand, single-table design keyed by entity access patterns. Ported so far: [`aws/db-identity/`](../aws/db-identity/) (`UserService`/`ApiKeyService`/storage layer of `SessionService`, 32 tests — `SessionService`'s JWT logic and audit-log-dependent methods excluded, see its README), [`aws/db-apps/`](../aws/db-apps/) (core of `AppService`, 26 tests — trending/popular ranking, fork-detachment, and comments/likes explicitly not ported), [`aws/db-model-config/`](../aws/db-model-config/) (`ModelProvidersService` + storage layer of `ModelConfigService`, 16 tests), and [`aws/db-analytics/`](../aws/db-analytics/) (most of `AnalyticsService`, 9 tests — `batchGetAppStats` excluded) against [docs/aws-dynamodb-schema.md](aws-dynamodb-schema.md). `ModelTestService` doesn't touch the database at all (an LLM-calling wrapper misplaced in this folder) — nothing to port there. Remaining: `AuthService` (1178 lines, largest and most security-critical of the nine — OAuth flows, password reset, lockout) and the audit-log table (Table 5) that `SessionService`'s excluded methods depend on. | Medium-High — no relational engine, so this is a query-layer rewrite, not a dialect swap (see decision above) |
| R2 | S3 | Low |
