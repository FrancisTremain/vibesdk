...
| D1 + Drizzle | DynamoDB on-demand, single-table design keyed by entity access patterns. First slice ported: [`aws/db-identity/`](../aws/db-identity/) (`UserService`/`ApiKeyService`, 20 tests) against [docs/aws-dynamodb-schema.md](aws-dynamodb-schema.md). Remaining 7 services (`AppService`, `AuthService`, `SessionService`, `AnalyticsService`, `ModelConfigService`, `ModelProvidersService`, `ModelTestService`) not yet ported. | Medium-High — no relational engine, so this is a query-layer rewrite, not a dialect swap (see decision above) |
| R2 | S3 | Low |
