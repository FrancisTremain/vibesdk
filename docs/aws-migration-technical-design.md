...
| D1 + Drizzle | DynamoDB on-demand, single-table design keyed by entity access patterns. Ported so far: [`aws/db-identity/`](../aws/db-identity/) (`UserService`/`ApiKeyService`, 20 tests) and [`aws/db-apps/`](../aws/db-apps/) (core of `AppService`, 26 tests — trending/popular ranking, fork-detachment, and comments/likes explicitly not ported, see its README) against [docs/aws-dynamodb-schema.md](aws-dynamodb-schema.md). Remaining 6 services (`AuthService`, `SessionService`, `AnalyticsService`, `ModelConfigService`, `ModelProvidersService`, `ModelTestService`) not yet ported. | Medium-High — no relational engine, so this is a query-layer rewrite, not a dialect swap (see decision above) |
| R2 | S3 | Low |
