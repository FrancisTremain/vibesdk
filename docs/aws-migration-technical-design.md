...
| D1 + Drizzle | DynamoDB on-demand, single-table design keyed by entity access patterns. Ported so far: [`aws/db-identity/`](../aws/db-identity/) (`UserService`/`ApiKeyService`/storage layer of `SessionService`, 32 tests), [`aws/db-apps/`](../aws/db-apps/) (core of `AppService`, 26 tests — trending/popular ranking, fork-detachment, comments/likes explicitly not ported), [`aws/db-model-config/`](../aws/db-model-config/) (`ModelProvidersService` + storage layer of `ModelConfigService`, 16 tests), [`aws/db-analytics/`](../aws/db-analytics/) (most of `AnalyticsService`, 9 tests — `batchGetAppStats` excluded), and [`aws/db-auth-flows/`](../aws/db-auth-flows/) (OAuth state/auth attempts/password reset/email verification/OTP storage primitives, 17 tests) against [docs/aws-dynamodb-schema.md](aws-dynamodb-schema.md) — Tables 1-4 all now validated by real code. `ModelTestService` doesn't touch the database at all (an LLM-calling wrapper misplaced in this folder) — nothing to port there. [`aws/auth-crypto/`](../aws/auth-crypto/) ports `PasswordService` and `validatePassword` (15 tests) — pure crypto/logic, no table of its own, the only required change being `crypto.subtle.timingSafeEqual` (a Cloudflare-only Web Crypto extension) swapped for `node:crypto`'s `timingSafeEqual`. [`aws/oauth-clients/`](../aws/oauth-clients/) ports `BaseOAuthProvider`/`GitHubOAuthProvider`/`GoogleOAuthProvider` (15 tests) — pure `fetch`-based OAuth 2.0 + PKCE, no table of its own, changes limited to swapping the Sentry-wired `createLogger` for a minimal injectable `Logger` interface and reading credentials as plain arguments instead of Cloudflare's `Env` binding type. [`aws/auth-orchestration/`](../aws/auth-orchestration/) ports `AuthService` itself (1178 lines — `register`/`login`/`logout`, OAuth login and account linking, OTP email verification, token validation), assembling all four packages above via real `file:` npm dependencies rather than duplicating them (the one package in this migration that isn't fully self-contained — see its README for why, and for the `constructor.name`-vs-`instanceof` fake-DynamoDB gotcha that composition surfaced). Building it added `OAuthIdentityStore` to `aws/db-identity` (the multi-provider-linking table the schema doc had designed but nothing needed yet) and gave `UserStore.createUser`/`createSession` an optional pre-generated `id` parameter (a real bug — JWT `sessionId` claims not matching persisted sessions — caught by a failing integration test, not inspection). 39 tests, covering full register/login/logout round trips, the OAuth account-takeover-prevention path, CSRF nonce/state-replay rejection, and account-linking edge cases. Every storage, crypto, OAuth, and orchestration piece `AuthService` depends on is now ported. Remaining: the audit-log table (Table 5, small, not yet built) and the primary-provider repoint on unlink (a real, acknowledged gap — see `aws/auth-orchestration`'s README). | Medium-High — no relational engine, so this is a query-layer rewrite, not a dialect swap (see decision above) |
| R2 | S3 | Low |
