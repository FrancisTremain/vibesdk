...
| D1 + Drizzle | DynamoDB on-demand, single-table design keyed by entity access patterns. Ported so far: [`aws/db-identity/`](../aws/db-identity/) (`UserService`/`ApiKeyService`, 20 tests), [`aws/db-apps/`](../aws/db-apps/) (core of `AppService`, 26 tests — trending/popular ranking, fork-detachment, and comments/likes explicitly not ported, see its README), [`aws/db-model-config/`](../aws/db-model-config/) (`ModelProvidersService` + the storage layer of `ModelConfigService`, 16 tests), and [`aws/db-analytics/`](../aws/db-analytics/) (most of `AnalyticsService`, 9 tests — `batchGetAppStats` excluded, depends on `db-apps`'s already-flagged fork/like gaps) against [docs/aws-dynamodb-schema.md](aws-dynamodb-schema.md). `ModelTestService` doesn't touch the database at all (an LLM-calling wrapper misplaced in this folder) — nothing to port there. Remaining 2 services (`AuthService`, `SessionService`) not yet ported — both are the largest and most security-critical of the nine. | Medium-High — no relational engine, so this is a query-layer rewrite, not a dialect swap (see decision above) |
| R2 | S3 | Low |
