/** In-memory fakes for the ECS/EC2 SDK client subset EcsRunner uses. Identical shape to aws/sandbox-orchestrator-lambda/src/fake-ecs.ts. */
...
export class FakeEcsClient {
	...
	async send(command: unknown): Promise<unknown> {
		...
		const taskArn = this.opts.taskArn ?? 'arn:aws:ecs:ap-southeast-2:111111111111:task/vibesdk-harness/fake';
