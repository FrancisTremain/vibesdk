/**
 * Duplicated snapshot of worker/agents/inferutils/config.ts, per
 * aws/model-config-defaults's README -- see there for why this is a
 * copy rather than a shared import, and the maintenance implication
 * (it can drift from the original; revisit as a real extraction during
 * Phase 6 cutover prep, not before).
 *
 * One substantive change: the original reads `env.PLATFORM_MODEL_PROVIDERS`
 * via Cloudflare's `cloudflare:workers` module-level `env` singleton.
 * That's a Workers-only import with no Node/Lambda equivalent --
 * replaced with a plain `process.env` read inside a function, called
 * at request time instead of module load time (a Lambda's environment
 * doesn't change between invocations, but reading it per-call avoids
 * relying on module-load-order the way the original's top-level
 * `env`-dependent constant did).
 */
import {
    AgentActionKey,
    AgentConfig,
...
export const AGENT_CONFIG: AgentConfig = process.env.PLATFORM_MODEL_PROVIDERS
