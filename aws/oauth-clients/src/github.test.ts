/**
 * Ported verbatim from worker/services/oauth/github.test.ts -- the
 * original had no Cloudflare-specific dependency, so no changes needed.
 */
...
describe('GitHubOAuthProvider.getUserInfo', () => {
	...
	it('throws when no email can be resolved', async () => {
		...
		await expect(createProvider().getUserInfo('token')).rejects.toThrow(/Could not retrieve user email/);
	});
});

describe('GitHubOAuthProvider.create', () => {
	afterEach(() => {
		vi.restoreAllMocks();
	});

	it('throws when credentials are missing', () => {
		expect(() => GitHubOAuthProvider.create(undefined, undefined, 'https://app.local')).toThrow(
			/GitHub OAuth credentials not configured/,
		);
	});

	it('builds the redirect URI from the base URL', () => {
		const provider = GitHubOAuthProvider.create('id', 'secret', 'https://app.local');
