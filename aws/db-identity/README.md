...
of D1 — against the `vibesdk-identity` table designed in
...
and [`worker/database/services/ApiKeyService.ts`](../../worker/database/services/ApiKeyService.ts).
...
single-item Put/Update/Delete. 20 tests cover user creation and lookup,
username reassignment and its uniqueness guarantee, AI Gateway
preference defaulting, session creation/lookup/expiry, and API key
CRUD including hash-uniqueness enforcement.
